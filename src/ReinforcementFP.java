...
//            }
    ...
    public ArrayList<Object> solve() throws IloException {
        ...
        while (!stop && maxiter <= 100) {
            ...
            if(poolpoint.size() > 0) inpool = FButils.L2norm(poolpoint, xh, numberOfIntegerVariables) <= eps;
            if((proximity && count == 1)){
                //System.out.println("done "+ FButils.L2norm(localSearch(xh), xh, numberOfIntegerVariables));
                poolpoint = xh;
                xh = localSearch(xh);
                G = G(xh);
                olderpoint = xh;
                ...
                //System.out.println("always in pool  "+ FButils.L2norm(localSearch(olderpoint), xh, numberOfIntegerVariables)+
                  //   "   " + Math.pow(FButils.L2Norm(xh),2));
                xh = localSearch(olderpoint);
                olderpoint = xh;
                G = G(xh);
                valueobj = 1d + scalarProdInt(G, xh);
            } else if (!proximity){
